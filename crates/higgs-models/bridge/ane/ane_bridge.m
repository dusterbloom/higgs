...
// Cache path changed from ~/.nanobot/ to ~/.config/higgs/
// ane_bridge.m — Objective-C ANE bridge for Higgs inference server
...
static NSString *ane_cache_dir(void) {
    if (!g_cache_dir) {
        ...
        g_cache_dir = [home stringByAppendingPathComponent:@".config/higgs/ane_cache"];
    ...
    int n_outputs, const size_t *output_sizes)
{
    @autoreleasepool {
        ...
        // so we maintain our own cache at ~/.config/higgs/ane_cache/<hexId>/net.plist.
