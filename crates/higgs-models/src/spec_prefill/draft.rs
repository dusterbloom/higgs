...
pub struct DraftModelConfig {
    ...
    pub top_p: f32,
    /// Smoothing kernel size for avg_pool1d (default 13, from paper).
    ...
    pub fn score_tokens(&self, tokens: &[u32]) -> Result<Array, ModelError> {
        match &self.model {
            AnyModel::Qwen3Next(model) => {
                ...
                scoring_attention::score_tokens_with_attention(
                    &mut model_clone,
                    tokens,
                    &self.config,
                )
            ...
            _ => {
                // Fallback to uniform scoring for non-Qwen3.5 models
                let n_tokens = tokens.len();
                let uniform = 1.0 / n_tokens as f32;
                Array::from_full(&[n_tokens as i32], uniform)
