...
impl Qwen3NextCausalLM {
    pub fn new(args: Qwen3NextModelArgs) -> Result<Self, Exception> {
        ...
        let mtp = if args.mtp_num_hidden_layers > 0 && !args.use_dense_mtp && !args.use_moe_mtp {
        ...
        let moe_mtp = if args.mtp_num_hidden_layers > 0 && args.use_moe_mtp {
            Some(MoeMtpHead::new(&args, ql, qb)?)
        } else {
            None
        };
