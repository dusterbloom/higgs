...
pub fn gen_bonsai_attention_projection(
    ...
    let _ = writeln!(m, "        tensor<int32, [4]> osa = const()[name=string(\"osa\"), val=tensor<int32, [4]>([1,{attn_dim},1,{seq}])];");
    let _ = writeln!(m, "        tensor<fp16, [1,{attn_dim},1,{seq}]> af = reshape(shape=osa,x=at)[name=string(\"ra\")];");
    let _ = writeln!(m, "        tensor<fp32, [1,{attn_dim},1,{seq}]> y = cast(dtype=to32,x=af)[name=string(\"cout\")];");
    let _ = writeln!(m, "    }} -> (y);");
    m.push_str("}\n");
...
pub fn gen_diffusion_ffn(
    ...
    // === RMSNorm (FFN) ===
    let _ = writeln!(m, "        tensor<fp16, [1,{dim},1,{seq}]> rn_sq = mul(x=xh,y=xh)[name=string(\"rnsq\")];");
    let _ = writeln!(m, "        tensor<fp16, [1,1,1,{seq}]> rn_m = reduce_mean(x=rn_sq,axes=ch_ax,keep_dims=kd)[name=string(\"rnm\")];");
    let _ = writeln!(m, "        tensor<fp16, [1,1,1,{seq}]> rn_e = add(x=rn_m,y=eps_v)[name=string(\"rne\")];");
    let _ = writeln!(m, "        tensor<fp16, [1,1,1,{seq}]> rn_r = pow(x=rn_e,y=nhalf)[name=string(\"rnr\")];");
    let _ = writeln!(m, "        tensor<fp16, [1,{dim},1,{seq}]> rn_n = mul(x=xh,y=rn_r)[name=string(\"rnn\")];");
    let _ = writeln!(m, "        tensor<fp16, [1,{dim},1,1]> rn_w = const()[name=string(\"rnw\"), val=tensor<fp16, [1,{dim},1,1]>(BLOBFILE(path=string(\"@model_path/weights/rms_ffn.bin\"), offset=uint64(64)))];");
    let _ = writeln!(m, "        tensor<fp16, [1,{dim},1,{seq}]> xnorm = mul(x=rn_n,y=rn_w)[name=string(\"xnorm\")];");

    // === FFN ===
    let _ = writeln!(m, "        tensor<int32, [4]> r2d = const()[name=string(\"r2d\"), val=tensor<int32, [4]>([1,1,{dim},{seq}])];");
    let _ = writeln!(m, "        tensor<fp16, [1,1,{dim},{seq}]> fn2 = reshape(shape=r2d,x=xnorm)[name=string(\"fn2\")];");
    let _ = writeln!(m, "        tensor<fp16, [1,1,{seq},{dim}]> fnt = transpose(perm=pm,x=fn2)[name=string(\"fnt\")];");
...
mod tests {
    ...
    func main<ios18>(tensor<fp32, [1, {c_in}, 1, {seq}]> x) {{\n\
        ...
        match AneKernel::compile_multi_weights(
            &mil, &name_ptrs, &blob_refs,
            &[c_in * seq * 4], &[c_out * seq * 4],
        ) {
            Ok(kernel) => {
    ...
    fn test_multi_dispatch_vs_fused() {
        ...
        let wq_blob = build_weight_blob_transposed(&lw.q_proj, cfg.heads * cfg.head_dim, cfg.hidden);
        let wk_blob = build_weight_blob_transposed(&lw.k_proj, cfg.kv_heads * cfg.head_dim, cfg.hidden);
        let wv_blob = build_weight_blob_transposed(&lw.v_proj, cfg.kv_heads * cfg.head_dim, cfg.hidden);
        let wo_blob = build_weight_blob_transposed(&lw.o_proj, cfg.hidden, cfg.heads * cfg.head_dim);
        ...
        let fused_blobs: Vec<&[u8]> = vec![
            &rms_att_blob, &rms_ffn_blob, &wq_blob, &wk_blob, &wv_blob, &wo_blob,
            &gate_blob, &up_blob, &down_blob, &rope_cos_blob, &rope_sin_blob,
            &q_norm_blob, &k_norm_blob,
        ];
        ...
        let fused_kernel = AneKernel::compile_multi_weights(
            &fused_mil.mil_text, &fused_names, &fused_blobs,
            &[fused_mil.input_bytes], &[fused_mil.output_bytes],
        ...
        let attn_mil = gen_diffusion_attention(
            cfg.hidden, cfg.heads, cfg.kv_heads, cfg.head_dim, seq, 1e-6,
        );
        let attn_blobs: Vec<&[u8]> = vec![
            &rms_att_blob, &wq_blob, &wk_blob, &wv_blob, &wo_blob,
            &rope_cos_blob, &rope_sin_blob, &q_norm_blob, &k_norm_blob,
        ];
        ...
        let attn_kernel = AneKernel::compile_multi_weights(
            &attn_mil.mil_text, &attn_names, &attn_blobs,
            &[attn_mil.input_bytes], &[attn_mil.output_bytes],
        ...
        let ffn_blobs: Vec<&[u8]> = vec![
            &rms_ffn_blob, &gate_blob, &up_blob, &down_blob,
        ];
        ...
        let ffn_kernel = AneKernel::compile_multi_weights(
            &ffn_mil.mil_text, &ffn_names, &ffn_blobs,
            &[ffn_mil.input_bytes], &[ffn_mil.output_bytes],
        ).expect("FFN compile");
        ...
        // === 6. Compare outputs ===
        let fused_f32: Vec<f32> = fused_out.chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]])).collect();
        let multi_f32: Vec<f32> = multi_out.chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]])).collect();

        let max_err = fused_f32.iter().zip(multi_f32.iter())
            .map(|(a, b)| (a - b).abs())
            .fold(0.0f32, f32::max);
        let mean_err = fused_f32.iter().zip(multi_f32.iter())
            .map(|(a, b)| (a - b).abs())
            .sum::<f32>() / fused_f32.len() as f32;
        ...
        assert!(max_err < 0.01, "Multi-dispatch output diverges from fused: max_err={max_err}");
    ...
    fn test_dim2048_attention_compile() {
        ...
        let blobs: Vec<&[u8]> = vec![&rms_att, &wq, &wk, &wv, &wo, &rope_cos, &rope_sin, &q_norm, &k_norm];
        let names: Vec<&str> = mil.weight_names.iter().map(|s| s.as_str()).collect();
        let total_mb: f64 = blobs.iter().map(|b| b.len() as f64).sum::<f64>() / 1e6;
        eprintln!("Total BLOBFILE size: {total_mb:.1}MB (limit: 32MB)");

        match AneKernel::compile_multi_weights(
            &mil.mil_text, &names, &blobs,
            &[mil.input_bytes], &[mil.output_bytes],
        ) {
            Ok(kernel) => {
                eprintln!("COMPILED OK — dim=2048 attention fits in one dispatch!");
                // Quick eval test
                let act: Vec<f32> = (0..dim * seq).map(|i| ((i as f32) * 0.001).sin() * 0.01).collect();
                let act_bytes: Vec<u8> = act.iter().flat_map(|f| f.to_le_bytes()).collect();
                kernel.write_input(0, &act_bytes);
                kernel.eval().expect("eval");
                let mut out = vec![0u8; dim * seq * 4];
                kernel.read_output(0, &mut out);
                let out_f32: Vec<f32> = out.chunks_exact(4).map(|c| f32::from_le_bytes(c.try_into().unwrap())).collect();
