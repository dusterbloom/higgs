...
pub(crate) fn load_qwen_vl_model_from_value(
    ...
    let text_args = if raw.get("text_config").is_some() {
        crate::qwen3_next::load_qwen3_5_text_config_args_from_value(dir, raw)?
    } else {
        let wrapped = serde_json::json!({ "text_config": raw.clone() });
        crate::qwen3_next::load_qwen3_5_text_config_args_from_value(dir, &wrapped)?
    };
...
fn load_safetensor_weights(dir: &Path) -> Result<HashMap<String, Array>, ModelError> {
    ...
    let workspace_bytes = files.iter().try_fold(0_u64, |total, path| {
        ...
        total.checked_add(bytes).ok_or_else(|| {
            ModelError::LoadCapacity("Qwen-VL artifact workspace overflow".to_owned())
        })
    })?;
