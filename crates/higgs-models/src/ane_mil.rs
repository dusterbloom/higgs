...
/// Compute OC tiling so each BLOBFILE weight stays under a conservative
/// per-file budget.
///
/// Although the nominal per-BLOBFILE limit is 16 MiB, in practice the ANE
/// compiler is much more reliable when we leave headroom for the wrapper/header
...
pub fn compute_blobfile_tile_plan(ic: usize, oc: usize) -> TilePlan {
    ...
    let tile = ((max_oc / 128) * 128).max(128);
    let n_tiles = (oc + tile - 1) / tile;
    let last = oc - (n_tiles - 1) * tile;
    TilePlan {
        tile_size: tile,
...
pub fn gen_matmul_program(ic: usize, oc: usize, seq_len: usize) -> (String, usize, usize, usize) {
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{ic},1,{seq}]> ah = cast(dtype=to16,x=act)[name=string(\"cin_a\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{ic},1,{oc}]> wh = cast(dtype=to16,x=w)[name=string(\"cin_w\")];"
    );
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{ic},{seq}]> a2 = reshape(shape=ra,x=ah)[name=string(\"a2\")];"
    );
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{ic},{oc}]> W = reshape(shape=rw,x=wh)[name=string(\"W\")];"
    );
    ...
    let _ = writeln!(
        m,
        "        tensor<fp32, [1,{oc},1,{seq}]> y = cast(dtype=to32,x=yf)[name=string(\"cout\")];"
    );
    let _ = writeln!(m, "    }} -> (y);");
    m.push_str("}\n");
...
/// Header layout matches `build_weight_blob` from io.h.
...
pub fn build_causal_mask_blob(seq: usize) -> Vec<u8> {
    ...
    // ANE blob header (matches build_weight_blob)
    blob[0] = 1;
    blob[4] = 2;
    blob[64] = 0xEF;
    blob[65] = 0xBE;
...
pub fn gen_blobfile_matmul(ic: usize, oc: usize, seq_len: usize, name: &str) -> FusedMil {
    ...
    let _ = writeln!(
        m,
        "        tensor<fp32, [1,{oc},1,{seq}]> out = cast(dtype=to32,x=yf)[name=string(\"cout\")];"
    );
    let _ = writeln!(m, "    }} -> (out);");
    m.push_str("}\n");
...
pub fn gen_fused_rkv_proj(dim: usize, key_dim: usize, seq_len: usize) -> FusedMil {
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{dim},{key_dim}]> Wr = const()[name=string(\"Wr\"), val=tensor<fp16, [1,1,{dim},{key_dim}]>(BLOBFILE(path=string(\"@model_path/weights/wr.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{dim},{key_dim}]> Wk = const()[name=string(\"Wk\"), val=tensor<fp16, [1,1,{dim},{key_dim}]>(BLOBFILE(path=string(\"@model_path/weights/wk.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{dim},{key_dim}]> Wv = const()[name=string(\"Wv\"), val=tensor<fp16, [1,1,{dim},{key_dim}]>(BLOBFILE(path=string(\"@model_path/weights/wv.bin\"), offset=uint64(64)))];"
    );
    // Cast + reshape input
    let _ = writeln!(
        m,
    ...
    let _ = writeln!(
        m,
        "        tensor<int32, [4]> ri = const()[name=string(\"ri\"), val=tensor<int32, [4]>([1,1,{dim},{seq}])];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{dim},{seq}]> x2 = reshape(shape=ri,x=xh)[name=string(\"x2\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{seq},{dim}]> xt = transpose(perm=pm,x=x2)[name=string(\"xt\")];"
    );
    // 3 matmuls
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{seq},{key_dim}]> rm = matmul(transpose_x=bF,transpose_y=bF,x=xt,y=Wr)[name=string(\"rm\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{seq},{key_dim}]> km = matmul(transpose_x=bF,transpose_y=bF,x=xt,y=Wk)[name=string(\"km\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{seq},{key_dim}]> vm = matmul(transpose_x=bF,transpose_y=bF,x=xt,y=Wv)[name=string(\"vm\")];"
    );
    // Transpose back + reshape each to [1,key_dim,1,seq]
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{key_dim},{seq}]> rt = transpose(perm=pm,x=rm)[name=string(\"rt\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{key_dim},{seq}]> kt = transpose(perm=pm,x=km)[name=string(\"kt\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{key_dim},{seq}]> vt = transpose(perm=pm,x=vm)[name=string(\"vt\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<int32, [4]> ro = const()[name=string(\"ro\"), val=tensor<int32, [4]>([1,{key_dim},1,{seq}])];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{key_dim},1,{seq}]> rf = reshape(shape=ro,x=rt)[name=string(\"rf\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{key_dim},1,{seq}]> kf = reshape(shape=ro,x=kt)[name=string(\"kf\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{key_dim},1,{seq}]> vf = reshape(shape=ro,x=vt)[name=string(\"vf\")];"
    );
    // Concatenate r|k|v on channel axis → [1, 3*key_dim, 1, seq]
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{out_ch},1,{seq}]> cat = concat(values=(rf,kf,vf),axis=cax,interleave=bF)[name=string(\"cat\")];"
    );
    let _ = writeln!(
        m,
...
pub fn gen_fused_gate_up_proj(ic: usize, inter: usize, seq_len: usize) -> FusedMil {
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{out_ch},1,{seq}]> cat = concat(values=(gf,uf),axis=cax,interleave=bF)[name=string(\"cat\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp32, [1,{out_ch},1,{seq}]> out = cast(dtype=to32,x=cat)[name=string(\"cout\")];"
...
/// Generate a fused Gate+Up+SiLU projection: 2 matmuls plus activation in one
/// ANE dispatch.
///
/// Input:  `[1, ic, 1, seq]` fp32
/// Output: `[1, inter, 1, seq]` fp32 where `out = SiLU(gate) * up`
pub fn gen_fused_silu_gate_up_proj(ic: usize, inter: usize, seq_len: usize) -> FusedMil {
    let seq = seq_len.max(ANE_MIN_SPATIAL);
    ...
    let _ = writeln!(
        m,
        "        tensor<fp32, [1,{inter},1,{seq}]> out = cast(dtype=to32,x=gated)[name=string(\"cout\")];"
    );
    let _ = writeln!(m, "    }} -> (out);");
    m.push_str("}\n");
...
pub fn gen_full_layer_projections(
    hidden: usize,
    q_dim: usize,
    kv_dim: usize,
    inter: usize,
    ...
    let _ = writeln!(
        m,
        "    func main<ios18>(tensor<fp32, [1,{hidden},1,{seq}]> x) {{"
    );
    // Constants
    let _ = writeln!(
        m,
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{hidden},{q_dim}]> Wq = const()[name=string(\"Wq\"), val=tensor<fp16, [1,1,{hidden},{q_dim}]>(BLOBFILE(path=string(\"@model_path/weights/wq.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{hidden},{kv_dim}]> Wk = const()[name=string(\"Wk\"), val=tensor<fp16, [1,1,{hidden},{kv_dim}]>(BLOBFILE(path=string(\"@model_path/weights/wk.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{hidden},{kv_dim}]> Wv = const()[name=string(\"Wv\"), val=tensor<fp16, [1,1,{hidden},{kv_dim}]>(BLOBFILE(path=string(\"@model_path/weights/wv.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{hidden},{inter}]> Wg = const()[name=string(\"Wg\"), val=tensor<fp16, [1,1,{hidden},{inter}]>(BLOBFILE(path=string(\"@model_path/weights/wg.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{hidden},{inter}]> Wu = const()[name=string(\"Wu\"), val=tensor<fp16, [1,1,{hidden},{inter}]>(BLOBFILE(path=string(\"@model_path/weights/wu.bin\"), offset=uint64(64)))];"
    );
    // O and Down have different input dims but we include them for the BLOBFILE size test
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{q_dim},{hidden}]> Wo = const()[name=string(\"Wo\"), val=tensor<fp16, [1,1,{q_dim},{hidden}]>(BLOBFILE(path=string(\"@model_path/weights/wo.bin\"), offset=uint64(64)))];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{inter},{hidden}]> Wd = const()[name=string(\"Wd\"), val=tensor<fp16, [1,1,{inter},{hidden}]>(BLOBFILE(path=string(\"@model_path/weights/wd.bin\"), offset=uint64(64)))];"
    );
    // Cast + reshape input
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{hidden},1,{seq}]> xh = cast(dtype=to16,x=x)[name=string(\"cin\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<int32, [4]> ri = const()[name=string(\"ri\"), val=tensor<int32, [4]>([1,1,{hidden},{seq}])];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{hidden},{seq}]> x2 = reshape(shape=ri,x=xh)[name=string(\"x2\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,1,{seq},{hidden}]> xt = transpose(perm=pm,x=x2)[name=string(\"xt\")];"
    );
    ...
    let _ = writeln!(
        m,
        "        tensor<int32, [4]> rg = const()[name=string(\"rg\"), val=tensor<int32, [4]>([1,{inter},1,{seq}])];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{q_dim},1,{seq}]> qf = reshape(shape=rq,x=qt)[name=string(\"qf\")];"
    ...
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{inter},1,{seq}]> gf = reshape(shape=rg,x=gt)[name=string(\"gf\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{inter},1,{seq}]> uf = reshape(shape=rg,x=ut)[name=string(\"uf\")];"
    );
    // Concat all on channel axis
    let _ = writeln!(
        m,
        "        tensor<fp16, [1,{out_ch},1,{seq}]> cat = concat(values=(qf,kf,vf,gf,uf),axis=cax,interleave=bF)[name=string(\"cat\")];"
    );
    let _ = writeln!(
        m,
        "        tensor<fp32, [1,{out_ch},1,{seq}]> out = cast(dtype=to32,x=cat)[name=string(\"cout\")];"
...
mod tests {
    ...
    #[test]
    fn test_blobfile_tile_plan_uses_safe_bonsai_tiling() {
        let w13 = compute_blobfile_tile_plan(2048, 6144);
        assert_eq!(w13.tile_size, 3072);
        assert_eq!(w13.n_tiles, 2);
        assert_eq!(w13.last_actual, 3072);

        let w2 = compute_blobfile_tile_plan(6144, 2048);
        assert_eq!(w2.tile_size, 1024);
        assert_eq!(w2.n_tiles, 2);
        assert_eq!(w2.last_actual, 1024);
    }
    ...
    fn test_split_matmul_compiles_on_ane() {
        ...
        let kernel = AneKernel::compile(&mil, None, &[act_bytes, weight_bytes], &[out_bytes])
        ...
        kernel.write_input(
            1,
            &w.iter().flat_map(|f| f.to_le_bytes()).collect::<Vec<u8>>(),
        );
    ...
    fn test_blobfile_matmul_identity() {
        ...
        eprintln!(
            "MIL ({} bytes):\n{}",
            fused.mil_text.len(),
            &fused.mil_text[..200.min(fused.mil_text.len())]
        );
        ...
        assert!(
            max_err < 1.0,
            "BLOBFILE identity error too large: {max_err}"
        );
    }
    ...
    fn test_fused_rkv_blobfile() {
        ...
        let max_err_r = (0..dim)
            .map(|i| ((i + 1) as f32 - r[i]).abs())
            .fold(0.0f32, f32::max);
        let max_err_k = (0..dim)
            .map(|i| ((i + 1) as f32 - k[i]).abs())
            .fold(0.0f32, f32::max);
        let max_err_v = (0..dim)
            .map(|i| ((i + 1) as f32 - v[i]).abs())
            .fold(0.0f32, f32::max);
    ...
    fn test_fused_diffusion_layer_benchmark() {
        ...
        for seq in [64, 128] {
            ...
            let wq: Vec<f32> = (0..hidden * q_dim)
                .map(|i| ((i as f32) * 1e-5).sin() * 0.01)
                .collect();
            let wk: Vec<f32> = (0..hidden * kv_dim)
                .map(|i| ((i as f32) * 1e-5).cos() * 0.01)
                .collect();
            let wv = wk.clone();
    ...
    fn test_blas_vs_ane_blobfile_latency() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (label, ic, oc) in [
            ...
            eprintln!("======================================================================");
            eprintln!(
                "   seq |  BLAS(CPU)           |  ANE(DMA)            |  GPU(Metal)          | best"
            );
            eprintln!(
                "-------+----------------------+----------------------+----------------------+------"
            );
            ...
            for seq in [1, 16, 32, 64, 128, 256, 512] {
                ...
                let winner = if min_us == blas_us {
                    "BLAS"
                } else if min_us == ane_us {
                    "ANE"
                } else {
                    "GPU"
                };
