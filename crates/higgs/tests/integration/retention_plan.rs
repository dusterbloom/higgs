...
fn planner_uses_complete_target_and_draft_geometry() {
    ...
    let config = r#"{"max_position_embeddings":65536,"num_hidden_layers":8,"num_key_value_heads":4,"head_dim":128,"hidden_size":1024,"num_attention_heads":8}"#;
    std::fs::write(target.join("config.json"), config).unwrap();
    std::fs::write(draft.join("config.json"), config).unwrap();
    let solo =
