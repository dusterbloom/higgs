...
impl MetricsStore {
    ...
    pub fn finalize_stream(&self, id: u64, output_tokens: u64, duration: Duration) {
        self.finish_stream(id, output_tokens, duration, None, None);
    }
    ...
    pub fn fail_stream(&self, id: u64, output_tokens: u64, duration: Duration, error: String) {
        self.finish_stream(id, output_tokens, duration, Some(500), Some(error));
    }

    /// Finalize a streaming request whose client disconnected.
    pub fn disconnect_stream(&self, id: u64, output_tokens: u64, duration: Duration) {
        self.finish_stream(
            id,
            output_tokens,
            duration,
