...
impl Engine {
    ...
    pub fn load_simple_with_draft<P: AsRef<Path>>(
        ...
        SimpleEngine::load_with_draft(dir, draft_dir, num_draft, kv_cache_config)
            .map(|e| Self::Simple(Box::new(e)))
    }

    #[cfg(feature = "ane")]
    pub fn load_simple_with_ane_draft<P: AsRef<Path>>(
        dir: P,
        draft_dir: P,
        num_draft: usize,
        kv_cache_config: KvCacheConfig,
    ) -> Result<Self, EngineError> {
        SimpleEngine::load_with_ane_draft(dir, draft_dir, num_draft, kv_cache_config)
            .map(|e| Self::Simple(Box::new(e)))
    }
